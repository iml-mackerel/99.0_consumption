...
pred.ga <- lapply(1:length(input.ga),function(x){boot.predator(input.ga[[x]],nsim=1,lower=0,deterministic=TRUE)})    # 2 sims for uniform distribution
names(pred.ga) <- names(input.ga)
...
p1 <- ggplot()+
    ...
    geom_ribbon(data=compare,aes(x=year,ymin=Clow,ymax=Cup),alpha=0.5)+
...
compare <- consum[consum$species=='northern_gannets' & 
                  consum$population %in% c('bonaventura','bird_rock','anticosti'),]
compare <- ddply(compare,c('year','sim'),summarise,C=sum(C))                                # sum over pops and lifestages
compare <- ddply(compare,c('year'),summarise,min=quantile(C,0.025),max=quantile(C,0.975))

p2 <- ggplot()+
    geom_ribbon(data=ref,aes(x=year,ymin=lower,ymax=upper),alpha=0.5,fill='darkgreen')+
    geom_ribbon(data=compare,aes(x=year,ymin=min,ymax=max),alpha=0.5)+
    labs(y='mackerel taken by gannets in the Gulf (t)')
